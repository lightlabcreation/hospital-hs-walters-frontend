...
const Assign = () => {
  const [jobs, setJobs] = useState([
    { id: 1, jobNumber: 'JOB-001', customer: 'ABC Corp', origin: 'Mumbai', destination: 'Delhi', status: 'Unassigned' },
    { id: 2, jobNumber: 'JOB-002', customer: 'XYZ Ltd', origin: 'Pune', destination: 'Bangalore', status: 'Unassigned' },
    { id: 3, jobNumber: 'JOB-003', customer: 'DEF Inc', origin: 'Delhi', destination: 'Mumbai', status: 'Unassigned' },
    { id: 4, jobNumber: 'JOB-004', customer: 'GHI Corp', origin: 'Chennai', destination: 'Hyderabad', status: 'Assigned' },
  ])

  const [drivers] = useState([
    { id: 1, name: 'John Doe', phone: '+91 9876543210', status: 'Available' },
    { id: 2, name: 'Jane Smith', phone: '+91 9876543211', status: 'Available' },
    { id: 3, name: 'Bob Wilson', phone: '+91 9876543212', status: 'Available' },
  ])
  ...
  const handleConfirmAssign = () => {
    if (!selectedDriver) {
      setErrors({ driver: 'Please select a driver' })
      return
    }
    const driver = drivers.find(d => d.id === parseInt(selectedDriver))
    setJobs(jobs.map(j => j.id === selectedJob.id ? { ...j, status: 'Assigned', driver: driver.name } : j))
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4" style={{ marginTop: '5.25rem' }}>
        <h1 className="text-2xl font-bold text-gray-800">Assign Driver</h1>
      </div>
